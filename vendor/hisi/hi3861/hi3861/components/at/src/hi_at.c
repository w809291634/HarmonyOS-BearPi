...
const hi_uart_attribute g_at_uart_cfg  = {38400, 8, 1, 0, 0};
...
{
    hi_u32 ret;
    LOG_D("[%s] buf:%s",__func__,buf);
    if (memcmp(buf, AT_ECHO_OFF, strlen(AT_ECHO_OFF)) == EOK || 
        memcmp(buf, AT_ECHO_ON, strlen(AT_ECHO_ON)) == EOK) {
        ret = at_cmd_process(buf);
        if ((ret != HI_ERR_SUCCESS) && (ret != HI_ERR_RECVING)) {
            g_at_ctrl.at_state = AT_IDLE;
        }
    } 
    else if (memcmp(buf, AT_CMD_HEADER, strlen(AT_CMD_HEADER)) == EOK) {
