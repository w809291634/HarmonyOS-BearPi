...
#ifdef USE_ZOENSION_AT
/*********************************************************************************************
* zonesion 自定义的AT命令
*********************************************************************************************/
t_rf_info rf_info={0};                               //通讯模块信息
int8_t g_rf_Link_status = 0;                         //wifi连接状态：0-未连接，1-连接wifi，2-连接智云 3-智云认证成功
int8_t g_led_Link_status = 0;                        //wifi连接状态，记录最大状态
...
void read_sys_parameter(void)
{
  ...
  ret = hi_factory_nv_read(NV_BASE_ID, &rf_info.base, sizeof(t_nv_base_info), 0);
  if (ret != HI_ERR_SUCCESS) {
    LOG_E("[%s] hi_factory_nv_read_base errno:%#x",__func__,ret);
    goto _error;
  }

  ret = hi_factory_nv_read(NV_WIFI_ID, &rf_info.wifi, sizeof(t_nv_wifi_info), 0);
  if (ret != HI_ERR_SUCCESS) {
    LOG_E("[%s] hi_factory_nv_read_wifi errno:%#x",__func__,ret);
    goto _error;
  }

  ret = hi_factory_nv_read(NV_ZHIYUN_ID, &rf_info.zhiyun, sizeof(t_nv_zhiyun_info), 0);
  if (ret != HI_ERR_SUCCESS) {
    LOG_E("[%s] hi_factory_nv_read_zhiyun errno:%#x",__func__,ret);
    goto _error;
  }
  return;
...
void write_sys_parameter(void)
{
  ...
  /* 再次读取写入的NV值 */
  ret = hi_factory_nv_write(NV_BASE_ID, &rf_info.base, sizeof(t_nv_base_info), 0);
  if (ret != HI_ERR_SUCCESS) {
    LOG_E("[%s] hi_factory_nv_read_base errno:%#x",__func__,ret);
    goto _error;
  }

  ret = hi_factory_nv_write(NV_WIFI_ID, &rf_info.wifi, sizeof(t_nv_wifi_info), 0);
  if (ret != HI_ERR_SUCCESS) {
    LOG_E("[%s] hi_factory_nv_read_wifi errno:%#x",__func__,ret);
    goto _error;
  }

  ret = hi_factory_nv_write(NV_ZHIYUN_ID, &rf_info.zhiyun, sizeof(t_nv_zhiyun_info), 0);
  if (ret != HI_ERR_SUCCESS) {
    LOG_E("[%s] hi_factory_nv_read_zhiyun errno:%#x",__func__,ret);
    goto _error;
  }
...
static hi_char * remove_marks(hi_char *data)
{
...
//AT+HW?
static hi_u32 at_query_hardware_info(hi_void)
{
  hi_at_printf("+HW:Hi3861\r\n");
  AT_RESPONSE_OK;
...
//AT+MAC?
static hi_u32 at_query_mac_info(hi_void)
{
  #undef  BUF_LEN
  #define BUF_LEN     50
  hi_char *buf = (hi_char*)malloc(BUF_LEN);
  if (buf == HI_NULL) {
      return HI_ERR_FAILURE;
  }

  snprintf(buf, BUF_LEN, "+MAC:%s\r\n", rf_info.wifi.mac);
  hi_at_printf(buf);
  free(buf);
  AT_RESPONSE_OK;
...
//AT+LINK?
static hi_u32 at_query_link_info(hi_void)
{
  #undef  BUF_LEN
  #define BUF_LEN     20

  int8_t link_status=0;
  hi_char *buf = (hi_char*)malloc(BUF_LEN);
  if (buf == HI_NULL) {
      return HI_ERR_FAILURE;
  }

  if(g_rf_Link_status==3) link_status=1;
  else link_status=0;
  sprintf(buf, "+LINK:%u\r\n", link_status);
  hi_at_printf(buf);
  free(buf);
...
//AT+SSID?
static hi_u32 at_query_ssid_info(hi_void)
{
  #undef  BUF_LEN
  #define BUF_LEN     50
  hi_char *buf = (hi_char*)malloc(BUF_LEN);
...
//AT+SSID=
static hi_u32 at_set_ssid_info(hi_s32 argc, const hi_char **argv)
{
  #undef  BUF_LEN
  #define BUF_LEN     40
  if (argc != 1) {
  ...
  if (buf == HI_NULL) {
    hi_at_printf("buffer malloc fail\r\n");
    return HI_ERR_FAILURE;
  }
  if(strlen(argv[0])>=BUF_LEN){
    free(buf);
    hi_at_printf("argv len error\r\n");
    return HI_ERR_FAILURE;
  }

  strncpy(buf,argv[0],strlen(argv[0]));
  buf = remove_marks(buf);
  //保存到变量中
  strncpy(rf_info.wifi.wifi_ssid, buf, sizeof(rf_info.wifi.wifi_ssid));
...
//AT+KEY?
static hi_u32 at_query_key_info(hi_void)
{
  #undef  BUF_LEN
  #define BUF_LEN     50
  hi_char *buf = (hi_char*)malloc(BUF_LEN);
  ...
  snprintf(buf,BUF_LEN, "+KEY:\"%s\"\r\n", rf_info.wifi.wifi_key);
  hi_at_printf(buf);
  free(buf);
  AT_RESPONSE_OK;
...
//AT+KEY=
static hi_u32 at_set_key_info(hi_s32 argc, const hi_char **argv)
{
  #undef  BUF_LEN
  #define BUF_LEN     20
  if (argc != 1) {
  ...
  buf = remove_marks(buf);
  //保存到变量中
  strncpy(rf_info.wifi.wifi_key, buf, sizeof(rf_info.wifi.wifi_key));
...
//AT+AID?
static hi_u32 at_query_aid_info(hi_void)
{
  #undef  BUF_LEN
  #define BUF_LEN     40
  hi_char *buf = (hi_char*)malloc(BUF_LEN);
  ...
  snprintf(buf,BUF_LEN, "+AID:%s\r\n", rf_info.zhiyun.zhiyun_id);
  hi_at_printf(buf);
  free(buf);
  AT_RESPONSE_OK;
...
//AT+AID=
static hi_u32 at_set_aid_info(hi_s32 argc, const hi_char **argv)
{
  #undef  BUF_LEN
  #define BUF_LEN     40
  if (argc > 1) {
  ...
  buf = remove_marks(buf);
  //保存到变量中
  strncpy(rf_info.zhiyun.zhiyun_id, buf, sizeof(rf_info.zhiyun.zhiyun_id));
...
//AT+AKEY?
static hi_u32 at_query_akey_info(hi_void)
{
  #undef  BUF_LEN
  #define BUF_LEN     120
  hi_char *buf = (hi_char*)malloc(BUF_LEN);
  ...
  snprintf(buf,BUF_LEN, "+AKEY:\"%s\"\r\n", rf_info.zhiyun.zhiyun_key);
  hi_at_printf(buf);
  free(buf);
  AT_RESPONSE_OK;
...
//AT+AKEY=
static hi_u32 at_set_akey_info(hi_s32 argc, const hi_char **argv)
{
  #undef  BUF_LEN
  #define BUF_LEN     120
  if (argc > 1) {
  ...
  buf = remove_marks(buf);
  //保存到变量中
  strncpy(rf_info.zhiyun.zhiyun_key, buf, sizeof(rf_info.zhiyun.zhiyun_key));
...
//AT+SIP?
static hi_u32 at_query_sip_info(hi_void)
{
  #undef  BUF_LEN
  #define BUF_LEN     60
  hi_char *buf = (hi_char*)malloc(BUF_LEN);
  ...
  if(rf_info.zhiyun.port != ZHIYUN_DEFAULT_PORT && rf_info.zhiyun.port != DEFAULT_UDP_PORT) {
    snprintf(buf,BUF_LEN, "+SIP:%s,%u\r\n", rf_info.zhiyun.ip, rf_info.zhiyun.port);
  } else {
    snprintf(buf,BUF_LEN, "+SIP:%s\r\n", rf_info.zhiyun.ip);
  }
...
//AT+SIP=
static hi_u32 at_set_sip_info(hi_s32 argc, const hi_char **argv)
{
  #undef  BUF_LEN
  #define BUF_LEN     120
  ...
  if (argc == 1) {
    ...
    strncpy(buf,argv[0],strlen(argv[0]));
    buf = remove_marks(buf);
    strncpy(rf_info.zhiyun.ip, buf, sizeof(rf_info.zhiyun.ip));

    //参数2
    if(rf_info.base.link_type==0)
      rf_info.zhiyun.port = ZHIYUN_DEFAULT_PORT;
    if(rf_info.base.link_type==1)
    ...
    if(strlen(argv[0])>=BUF_LEN || strlen(argv[1])>=BUF_LEN){
      free(buf);
      hi_at_printf("argv len error\r\n");
      return HI_ERR_FAILURE;
    }
    ...
    //参数2
    rf_info.zhiyun.port = atoi(argv[1]);

    free(buf);
    AT_RESPONSE_OK;
  }
  else return HI_ERR_FAILURE;

  return HI_ERR_SUCCESS;
}

//AT+ENVSAVE
hi_void at_save_parameter_cmd(hi_void)
{
    AT_RESPONSE_OK;
    write_sys_parameter();
}
...
//ATE0
extern hi_u8 g_at_echo;
static hi_u32 at_ate0_cmd(hi_void)
{ 
    g_at_echo = 0;
    AT_RESPONSE_OK;
    return HI_ERR_SUCCESS;
...
//ATE1
static hi_u32 at_ate1_cmd(hi_void)
{
    g_at_echo = 1;
    AT_RESPONSE_OK;
    return HI_ERR_SUCCESS;
...
const at_cmd_func g_at_general_func_tbl[] = {
    ...
    {"+SMAC", 5, HI_NULL, (at_call_back_func)cmd_get_macaddr, (at_call_back_func)cmd_set_macaddr, HI_NULL},
...
#ifdef USE_ZOENSION_AT
    /* zonesion */
    {"+HW", 3, HI_NULL, (at_call_back_func)at_query_hardware_info, HI_NULL, HI_NULL},
    {"+MAC", 4, HI_NULL, (at_call_back_func)at_query_mac_info, HI_NULL, HI_NULL},
    {"+LINK", 5, HI_NULL, (at_call_back_func)at_query_link_info, HI_NULL, HI_NULL},
    {"+SSID", 5, HI_NULL, (at_call_back_func)at_query_ssid_info, (at_call_back_func)at_set_ssid_info, HI_NULL},
    {"+KEY", 4, HI_NULL, (at_call_back_func)at_query_key_info, (at_call_back_func)at_set_key_info, HI_NULL},
    {"+AID", 4, HI_NULL, (at_call_back_func)at_query_aid_info, (at_call_back_func)at_set_aid_info, HI_NULL},
    {"+AKEY", 5, HI_NULL, (at_call_back_func)at_query_akey_info, (at_call_back_func)at_set_akey_info, HI_NULL},
    {"+SIP", 4, HI_NULL, (at_call_back_func)at_query_sip_info, (at_call_back_func)at_set_sip_info, HI_NULL},
    {"+ENVSAVE", 8, HI_NULL, HI_NULL, HI_NULL, (at_call_back_func)at_save_parameter_cmd},
    {"+RESET", 6, HI_NULL, HI_NULL, HI_NULL, (at_call_back_func)at_reset_cmd},
    {"ATE0", 4, HI_NULL, HI_NULL, HI_NULL, (at_call_back_func)at_ate0_cmd},
    {"ATE1", 4, HI_NULL, HI_NULL, HI_NULL, (at_call_back_func)at_ate1_cmd},
#endif
};
