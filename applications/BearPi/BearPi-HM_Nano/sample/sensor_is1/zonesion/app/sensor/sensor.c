...
static void updateA0(void) {
  ...
  uint8_t value = E53_IS1_Read_infrared(); 
  if(value==1){
    st = LOS_Tick2MS((UINT32)LOS_TickCountGet());
    A0 = 1;
  }
  else {
    if(LOS_Tick2MS((UINT32)LOS_TickCountGet()) - st > 3000) A0 = 0;
  }
}
...
void sensorUpdate(void) {
  ...
  if ((D0 & 0x01) == 0x01){                                      
    updateA0();
    snprintf(temp,sizeof(temp),"A0=%u,", A0);
...
void sensorCheck(void) {
  ...
  if (lastA0 != A0 || (ct0 != 0 && LOS_Tick2MS((UINT32)LOS_TickCountGet()) > (ct0+3000))) { 
    if (D0 & 0x01) {
      snprintf(temp,sizeof(temp), "A0=%u,", A0);
      strcat(data_send, temp);
      send_flag = 1;
      ct0 = LOS_Tick2MS((UINT32)LOS_TickCountGet());
    }
    if (A0 == 0) {
      ct0 = 0;
    }
