...
/***************************************************************
* 函数名称: E53_IA1_Read_BH1750
* 说    明: 读取光照度数据
* 参    数: 无
* 返 回 值: 无
***************************************************************/
float E53_IA1_Read_BH1750(void)
{
    Start_BH1750(); // 启动传感器采集数据
    usleep(180000);
    WifiIotI2cData bh1750_i2c_data = { 0 };
    ...
    bh1750_i2c_data.receiveLen = 2;
    I2cRead(WIFI_IOT_I2C_IDX_1, (BH1750_Addr<<1)|0x01,&bh1750_i2c_data);   // 读取传感器数据
    return (float)(((recv_data[0]<<8) + recv_data[1])/1.2);
}

/***************************************************************
* 函数名称: E53_IA1_Read_SHT3X
* 说    明: 读取温湿度
* 参    数: [out]Temperature 温度
* 参    数: [out]Humidity 湿度
* 返 回 值: 无
***************************************************************/
void E53_IA1_Read_SHT3X(float* Temperature,float* Humidity)
{
    uint8_t  data[3];    //data array for checksum verification
    uint16_t dat,tmp;
    uint8_t SHT3X_Data_Buffer[6]; 																		//byte 0,1 is temperature byte 4,5 is humidity
