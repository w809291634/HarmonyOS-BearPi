...
static char A0[50];                                               // A0存储6轴数据信息
static int16_t A1 = 0;                                            // A1存储倾斜角度
...
static void updateA0(void) {
  ...
  if(LOS_MuxPend(i2cbus_mutex, LOS_MS2Tick(1000)) != LOS_OK) return;
	...
	float Accel[3];
	if (MPU6050ReadID() == 0){	
    LOG_E("[%s] MPU6050ReadID error",__func__);
    return;
  }
  MPU6050ReadAcc_ex(Accel);
  usleep(50000);
  LOS_MuxPost(i2cbus_mutex);

  snprintf(A0,50,"%.1f&%.1f&%.1f",Accel[0],Accel[1],Accel[2]);
#else
...
void sensorUpdate(void) {
  ...
  if ((D0 & 0x01) == 0x01){                                     
    updateA0();
    snprintf(temp,sizeof(temp), "A0=%s,", A0);
    strcat(data_send, temp);
  }
  if ((D0 & 0x02) == 0x02){                                     
    updateA1();
    snprintf(temp,sizeof(temp), "A1=%d,", A1);
    strcat(data_send, temp);
  }
...
void sensorCheck(void) {
  ...
  if (lastA1 != A1 || (ct1 != 0 && LOS_Tick2MS((UINT32)LOS_TickCountGet()) > (ct1+3000))) { 
    if (D0 & 0x02) {
      snprintf(temp,sizeof(temp), "A1=%u,", A1);
      strcat(data_send, temp);
      send_flag = 1;
      ct1 = LOS_Tick2MS((UINT32)LOS_TickCountGet());
    }
    if (A1 == 0) {
      ct1 = 0;
    }
