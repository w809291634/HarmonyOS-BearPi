...
/***************************************************************
  * 函数功能: 读取MPU6050的加速度数据并转换
  * 输入参数: 无
  * 返 回 值: 无
  * 说    明: 无
***************************************************************/ 
void MPU6050ReadAcc_ex(float *Data)
{ 
    int16_t ax, ay, az;  
    uint8_t buf[6];
    MPU6050_ReadData(MPU6050_ACC_OUT, buf, 6);
    ...
    Data[0] = ax / 2048.0f * 9.8;  
    Data[1] = ay / 2048.0f * 9.8;  
    Data[2] = az / 2048.0f * 9.8;  
}
...
int lis3dh_angle(void)
{
    ...
    y = sy/10;
    z = sz/10;
    int a1 = atan2(x, z)*(180/3.1415926f);
    int a2 = atan2(y, z)*(180/3.1415926f);
    if (a1 < 0) a1 = -a1;
    if (a2 < 0) a2 = -a2;
    return a1>a2?a1:a2;
}
